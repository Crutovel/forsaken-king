...
class PlayerSoulKilling implements PlayerState {

    private Player player;
    private int killIndex;

    PlayerSoulKilling(Player player, int killIndex) {
        this.player = player;
    ...
    public void update() {
        ...
        if (player.graphics.currentSpriteIndex == killIndex &&
                player.graphics.frameCount == 0) {
            player.soulKillPerformed = true;
            player.soulKill();
        }
    }
